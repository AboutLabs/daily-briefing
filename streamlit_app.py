...
with col2:
    ...
    if not os.path.exists(reports_dir):
        st.warning(f"No reports directory found. Please generate a report first.")
        reports = []
    else:
        # List all .md reports in the reports directory
        reports = [report for report in os.listdir(reports_dir) if report.endswith('.md')]
    ...
    if reports:
        ...
        if selected_report:
            ...
            image_file = f'{reports_dir}/{report_base_name}.png'

            if st.button("Delete Selected Report"):
                # Check if the report file exists before attempting to delete
                if os.path.exists(f'{reports_dir}/{selected_report}'):
                    os.remove(f'{reports_dir}/{selected_report}')
